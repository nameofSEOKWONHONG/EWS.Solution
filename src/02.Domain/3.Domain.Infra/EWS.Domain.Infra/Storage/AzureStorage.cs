...
public class AzureStorage : ICloudStorage
{
    ...
    public async Task<bool> FileExist(string tenantId, string userId, string hashedFileName)
    {
        var client = _blobContainerClient.GetBlobClient($"{tenantId}/{userId}/{hashedFileName}");
    ...
    /// <returns></returns>
    public async Task<string> FileDownloadUrl(string tenantId, string fullUri, string fileName)
    {
    ...
    /// <param name="hashedFileName"></param>
    /// <returns></returns>
    public async Task<bool> FileDelete(string tenantId, string userId, string hashedFileName)
    {
        var client = _blobContainerClient.GetBlobClient($"{tenantId}/{userId}/{hashedFileName}");
    ...
    public async Task<JFileObject> FileUpload(string tenantId, string userId, JFileObject file)
    {
        var blobUri = $"{tenantId}/{userId}/{file.HashedFileName}";
