...
namespace EWS.Infrastructure.ObjectPool;
	...
	internal DefaultStringBuilderPool(int initialBuilderCapacity, int maxBuilderCapacity, int poolSize)
	{
		if (initialBuilderCapacity <= 0)
		{
			throw new ArgumentOutOfRangeException(
				...
				"Initial capacity of builder must be a positive value."
			);
		}
		...
		if (maxBuilderCapacity <= 0)
		{
			throw new ArgumentOutOfRangeException(
				...
				"Maximum capacity of builder must be a positive value."
			);
		}
		...
		if (poolSize <= 0)
		{
			throw new ArgumentOutOfRangeException(
				nameof(poolSize),
				"Pool size must be a positive value."
			);
		}
		...
		if (initialBuilderCapacity > maxBuilderCapacity)
		{
			throw new ArgumentOutOfRangeException(
				nameof(initialBuilderCapacity),
				string.Format("Initial capacity of builder must be less than or equal to the maximum capacity - {0}.",
					maxBuilderCapacity)
			);
		}
